...
const formats = {
  ...
  json: [enumerateErrorFormat(), format.timestamp({ format: process.env.LOG_TIME_FORMAT }), format.json()]
}
...
export default (moduleName: String) => {
  ...
  return {
    ...
    debugJSON: (label: String, msg: Object) => CustomLogger.logger.debug(`[${label}] ${JSON.stringify(msg, null, 2)}`, options)
  }
}
