...
export class HttpService {
  ...
  prepareAndCheckData (input: any): Buffer {
    ...
    input.entries.forEach((element: any) => {
      this.throwsIfEmpty('consumer', element.consumer)
      this.throwsIfEmpty('meter', element.meter)
      this.throwsIfEmpty('timestamp', element.timestamp)
      this.throwsIfEmpty('activePower', element.activePower)
      this.throwsIfEmpty('reactivePower', element.reactivePower)
      this.throwsIfEmpty('fullPower', element.fullPower)

      result = Buffer.concat([
        result,
        Buffer.from(element.consumer, 'utf-8'),
        Buffer.from(element.meter, 'utf-8'),
        Buffer.from(element.timestamp, 'utf-8'),
        Buffer.from(element.activePower.toString(), 'utf-8'),
        Buffer.from(element.reactivePower.toString(), 'utf-8'),
        Buffer.from(element.fullPower.toString(), 'utf-8')
      ])
    })
